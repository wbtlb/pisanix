...
impl ShardingRewrite {
    pub fn new(rules: Vec<Sharding>, endpoints: Vec<Endpoint>) -> Self {
        ShardingRewrite {
            rules, 
            raw_sql: "".to_string(), 
            endpoints 
    ...
    pub fn database_strategy(&self, meta: RewriteMetaData) -> Result<Vec<ShardingRewriteOutput>, Box<dyn std::error::Error>> {
    ...
    fn find_table_rule<'a>(&'a self, tables: &'a IndexMap<u8, Vec<TableIdent>>) -> Vec<(u8, Sharding, &TableIdent)> {
    ...
    pub fn get_meta(&self, ast: &mut SqlStmt) -> RewriteMetaData {
...
impl ShardingRewriter<SqlStmt> for ShardingRewrite {
...
mod test {
    ...
    fn get_table_sharding_config() -> (Vec<Sharding>, Vec<Endpoint>) {
        (
            vec![
                Sharding { 
                    table_name: "tshard".to_string(), 
                    actual_datanodes: vec!["ds0".to_string()], 
                    binding_tables: None, 
                    broadcast_tables: None, 
                    table_strategy: Some(
                        StrategyType::TableStrategyConfig(
                            crate::config::TableStrategyConfig {
                                ...
                                table_sharding_algorithm_name: ShardingAlgorithmName::CRC32Mod,
                                table_sharding_column: "order_id".to_string(),
                                sharding_count: 4,
                            },
                        ),
                    ),
    ...
    fn test_database_sharding_strategy() {
        ...
        let mut sr = ShardingRewrite::new(config.0.clone(), config.1.clone());
        ...
        let mut sr = ShardingRewrite::new(config.0.clone(), config.1);
        sr.set_raw_sql(raw_sql.clone());
        let mut ast = parser.parse(&raw_sql).unwrap();
        let meta = sr.get_meta(&mut ast[0]);
        let res = sr.database_strategy( meta).unwrap();
        assert_eq!(
    ...
    fn test_table_sharding_strategy() {
        ...
        let raw_sql = "SELECT idx from tshard where idx = 3".to_string();
        let parser = Parser::new();
        let mut ast = parser.parse(&raw_sql).unwrap();
        let mut sr = ShardingRewrite::new(config.0.clone(), config.1.clone());
        sr.set_raw_sql(raw_sql);
        ...
        println!("res >> {:#?}", res);
        // assert_eq!(res[0].target_sql, "SELECT idx from tshard_0000 where idx = 3");
    }
}